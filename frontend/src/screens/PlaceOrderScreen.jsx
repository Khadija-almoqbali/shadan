...
const PlaceOrderScreen = () => {
  ...
  const totalPrice =
    Math.max(0, itemsPrice - safeDiscount) + shippingPrice;
  ...
  const placeOrderHandler = async () => {
    try {
      ...
      toast.error(
        error?.data?.message || error?.error || t("order.error")
      );
    }
  };
        ...
        <Row>
              ...
              <ListGroup variant="flush">
                <ListGroup.Item className="luxury-item">
                  <p className="section-text">
                    <strong>{t("order.deliveryType")}:</strong>{" "}
                ...
                <ListGroup.Item className="luxury-item">
                  ...
                  {cart.cartItems.length === 0 ? (
                    <Message>{t("order.emptyCart")}</Message>
                  ) : (
                    <ListGroup variant="flush">
          ...
          <Col md={4} className="mt-4 mt-md-0">
            <Card className="summary-card border-0 sticky-summary">
              <Card.Body>
                <h2 className="summary-title mb-4">
                  {t("order.summary")}
                </h2>
